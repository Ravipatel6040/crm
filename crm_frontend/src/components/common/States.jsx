...
export function EmptyState({ icon: Icon = Inbox, title = "Nothing here yet", description, action }) {
  return (
    <div className="flex flex-col items-center justify-center text-center py-14 px-4">
      <div className="h-14 w-14 rounded-2xl bg-primary-50 dark:bg-primary-500/10 text-primary-400 dark:text-primary-400 flex items-center justify-center mb-4">
        <Icon size={26} />
      </div>
...
export function ErrorState({ title = "Something went wrong", description = "Please try again in a moment.", onRetry }) {
  return (
    <div className="flex flex-col items-center justify-center text-center py-14 px-4">
      <div className="h-14 w-14 rounded-2xl bg-red-50 dark:bg-red-500/10 text-red-400 flex items-center justify-center mb-4">
...
export function Skeleton({ className = "" }) {
  return <div className={`animate-pulse rounded-lg bg-slate-100 dark:bg-slate-700/50 ${className}`} />;
}
  ...
  return (
    <div className="rounded-xl border border-slate-100 dark:border-slate-700 overflow-hidden">
      {Array.from({ length: rows }).map((_, r) => (
        <div
          key={r}
          className="flex gap-4 px-4 py-3.5 border-b border-slate-50 dark:border-slate-700/50 last:border-0"
        >
