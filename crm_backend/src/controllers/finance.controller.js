...
export const createInvoice = asyncHandler(async (req, res) => {
  ...
  const invoice = await createWithUniqueNumber(
    Invoice,
    () => ({
      invoiceNumber: invoiceNumber || `INV-${new Date().getFullYear()}-${Date.now().toString().slice(-6)}`,
...
export const createPayment = asyncHandler(async (req, res) => {
  ...
  // Payment.client is a required field — a missing/invalid one must be
  // rejected outright, not silently swapped for null and left to Mongoose's
  // validator to throw an unhandled 500 on .create().
  if (!client || !mongoose.Types.ObjectId.isValid(client)) {
    throw new ApiError(400, "A valid client is required");
  }

  const payment = await createWithUniqueNumber(
    Payment,
    () => ({
      invoiceNumber: invoiceNumber || `PAY-${new Date().getFullYear()}-${Date.now().toString().slice(-6)}`,
      client,
      project: project && mongoose.Types.ObjectId.isValid(project) ? project : null,
