...
/**
 * Creates a document via `Model.create(buildDoc())`, regenerating the
 * auto-numbered field and retrying on a duplicate-key error (Mongo code
 * 11000) instead of trusting a `count()`-then-insert check, which has a
 * race window under concurrent requests. Only retries when the number was
 * auto-generated — a user-supplied number that collides should fail loudly,
 * not be silently replaced.
 */
const createWithUniqueNumber = async (Model, buildDoc, wasAutoGenerated, attempts = 5) => {
